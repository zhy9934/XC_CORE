...
////GBK->Utf8
//char* ObjectMgr::GBKToUtf8(const char* strGBK)
//{
...
bool bot_minion_ai::OnGossipHello(Player* player, Creature* creature, uint32 /*option*/)
{
	...
	if (player->GetGUIDLow() != creature->GetBotAI()->GetBotOwnerGuid())
	{
		if (creature->IsFreeBot() && !player->IsGameMaster())
		{
			...
			if (!reason)
			{
				...
				player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TAXI, sObjectMgr->GBKToUtf8("要和我决斗吗?"), GOSSIP_SENDER_HIRE, GOSSIP_ACTION_INFO_DEF + reason);
	...
	if (creature->GetBotAI()->GetBotOwnerGuid())
	{
		...
		{
			player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, sObjectMgr->GBKToUtf8("装备管理..."), GOSSIP_SENDER_EQUIPMENT, GOSSIP_ACTION_INFO_DEF + 1);
			player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, sObjectMgr->GBKToUtf8("角色管理..."), GOSSIP_SENDER_ROLES, GOSSIP_ACTION_INFO_DEF + 1);
			player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, sObjectMgr->GBKToUtf8("队伍管理..."), GOSSIP_SENDER_FORMATION, GOSSIP_ACTION_INFO_DEF + 1);
			player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, sObjectMgr->GBKToUtf8("释放技能..."), GOSSIP_SENDER_ABILITIES, GOSSIP_ACTION_INFO_DEF + 1);
			if (creature->GetBotClass() >= BOT_CLASS_EX_START)
				player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, "Abilities status...", GOSSIP_SENDER_SCAN_OWNER, GOSSIP_ACTION_INFO_DEF + 1);

			if (!gr)
				player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, sObjectMgr->GBKToUtf8("<创建队伍>"), GOSSIP_SENDER_JOIN_GROUP, GOSSIP_ACTION_INFO_DEF + 1);
			else if (!gr->IsMember(creature->GetGUID()))
				player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, sObjectMgr->GBKToUtf8("<添加队员>"), GOSSIP_SENDER_JOIN_GROUP, GOSSIP_ACTION_INFO_DEF + 2);
			else
				player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, sObjectMgr->GBKToUtf8("<踢出队伍>"), GOSSIP_SENDER_LEAVE_GROUP, GOSSIP_ACTION_INFO_DEF + 1);
		...
		if (player == creature->GetBotOwner() || (gr && gr->IsMember(creature->GetBotOwner()->GetGUID())))
		{
			...
			{
			case BOT_CLASS_MAGE:
				player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, sObjectMgr->GBKToUtf8("来点面包"), GOSSIP_SENDER_CLASS, GOSSIP_ACTION_INFO_DEF + 1);
				player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, sObjectMgr->GBKToUtf8("来点喝的"), GOSSIP_SENDER_CLASS, GOSSIP_ACTION_INFO_DEF + 2);
				menus = true;
				break;
			default:
				break;
		...
		if (player == creature->GetBotOwner())
		{
			...
			astr << sObjectMgr->GBKToUtf8("你真的打算放弃") << creature->GetName() << sObjectMgr->GBKToUtf8("吗? 你可别后悔哦!");
			player->PlayerTalkClass->GetGossipMenu().AddMenuItem(-1, GOSSIP_ICON_TAXI, sObjectMgr->GBKToUtf8("你被解雇了!"),
	...
	if (menus)
		player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, sObjectMgr->GBKToUtf8("哦,没事了."), 0, GOSSIP_ACTION_INFO_DEF + 1);
...
bool bot_minion_ai::OnGossipSelect(Player* player, Creature* creature/* == me*/, uint32 sender, uint32 action)
{
	...
	{
				BotSay(sObjectMgr->GBKToUtf8("没事别点我..."), player);
				break;
	}
	...
	case GOSSIP_SENDER_CLASS: //food/drink (classes: MAGE)
	{
								  ...
								  case BOT_CLASS_MAGE:
								  {
														 ...
														 if (!food)
														 {
															 std::string errorstr = sObjectMgr->GBKToUtf8("我还不会做");
															 errorstr += iswater ? sObjectMgr->GBKToUtf8("水") : sObjectMgr->GBKToUtf8("面包");
															 errorstr += sObjectMgr->GBKToUtf8(",不好意思.");
														 ...
														 if (result != SPELL_CAST_OK)
														 {
															 ...
															 delete foodspell;
															 BotWhisper(sObjectMgr->GBKToUtf8("现在我还不能做这件事."), player);
														 }
														 else
														 {
															 aftercastTargetGuid = player->GetGUID();
															 foodspell->prepare(&targets);
															 BotWhisper(sObjectMgr->GBKToUtf8("做好了，给你."), player);
														 }
														 break;
								  }
								  default:
	...
	case GOSSIP_SENDER_EQUIPMENT: //equips change s1: send what slots we can use
	{
									  ...
									  //general
									  player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, sObjectMgr->GBKToUtf8("查看装备"), GOSSIP_SENDER_EQUIPMENT_LIST, GOSSIP_ACTION_INFO_DEF + 1);

									  //auto-equip
									  player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, sObjectMgr->GBKToUtf8("自动装备..."), GOSSIP_SENDER_EQUIP_AUTOEQUIP, GOSSIP_ACTION_INFO_DEF + 1);

									  //weapons
									  player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, sObjectMgr->GBKToUtf8("主手武器..."), GOSSIP_SENDER_EQUIPMENT_SHOW, GOSSIP_ACTION_INFO_DEF + BOT_SLOT_MAINHAND);
									  if (_canUseOffHand())
										  player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, sObjectMgr->GBKToUtf8("副手武器..."), GOSSIP_SENDER_EQUIPMENT_SHOW, GOSSIP_ACTION_INFO_DEF + BOT_SLOT_OFFHAND);
									  if (_canUseRanged())
										  player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, sObjectMgr->GBKToUtf8("远程武器..."), GOSSIP_SENDER_EQUIPMENT_SHOW, GOSSIP_ACTION_INFO_DEF + BOT_SLOT_RANGED);
									  else
										  player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, sObjectMgr->GBKToUtf8("圣物..."), GOSSIP_SENDER_EQUIPMENT_SHOW, GOSSIP_ACTION_INFO_DEF + BOT_SLOT_RANGED);

									  //armor
									  player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, sObjectMgr->GBKToUtf8("头部..."), GOSSIP_SENDER_EQUIPMENT_SHOW, GOSSIP_ACTION_INFO_DEF + BOT_SLOT_HEAD);
									  player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, sObjectMgr->GBKToUtf8("颈部..."), GOSSIP_SENDER_EQUIPMENT_SHOW, GOSSIP_ACTION_INFO_DEF + BOT_SLOT_NECK);
									  player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, sObjectMgr->GBKToUtf8("肩部..."), GOSSIP_SENDER_EQUIPMENT_SHOW, GOSSIP_ACTION_INFO_DEF + BOT_SLOT_SHOULDERS);
									  player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, sObjectMgr->GBKToUtf8("胸部..."), GOSSIP_SENDER_EQUIPMENT_SHOW, GOSSIP_ACTION_INFO_DEF + BOT_SLOT_CHEST);
									  player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, sObjectMgr->GBKToUtf8("腰部..."), GOSSIP_SENDER_EQUIPMENT_SHOW, GOSSIP_ACTION_INFO_DEF + BOT_SLOT_WAIST);
									  player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, sObjectMgr->GBKToUtf8("腿部..."), GOSSIP_SENDER_EQUIPMENT_SHOW, GOSSIP_ACTION_INFO_DEF + BOT_SLOT_LEGS);
									  player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, sObjectMgr->GBKToUtf8("脚..."), GOSSIP_SENDER_EQUIPMENT_SHOW, GOSSIP_ACTION_INFO_DEF + BOT_SLOT_FEET);
									  player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, sObjectMgr->GBKToUtf8("手腕..."), GOSSIP_SENDER_EQUIPMENT_SHOW, GOSSIP_ACTION_INFO_DEF + BOT_SLOT_WRIST);
									  player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, sObjectMgr->GBKToUtf8("手..."), GOSSIP_SENDER_EQUIPMENT_SHOW, GOSSIP_ACTION_INFO_DEF + BOT_SLOT_HANDS);
									  player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, sObjectMgr->GBKToUtf8("背部..."), GOSSIP_SENDER_EQUIPMENT_SHOW, GOSSIP_ACTION_INFO_DEF + BOT_SLOT_BACK);
									  player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, sObjectMgr->GBKToUtf8("衬衣..."), GOSSIP_SENDER_EQUIPMENT_SHOW, GOSSIP_ACTION_INFO_DEF + BOT_SLOT_BODY);
									  player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, sObjectMgr->GBKToUtf8("戒指1..."), GOSSIP_SENDER_EQUIPMENT_SHOW, GOSSIP_ACTION_INFO_DEF + BOT_SLOT_FINGER1);
									  player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, sObjectMgr->GBKToUtf8("戒指2..."), GOSSIP_SENDER_EQUIPMENT_SHOW, GOSSIP_ACTION_INFO_DEF + BOT_SLOT_FINGER2);
									  player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, sObjectMgr->GBKToUtf8("饰品1..."), GOSSIP_SENDER_EQUIPMENT_SHOW, GOSSIP_ACTION_INFO_DEF + BOT_SLOT_TRINKET1);
									  player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, sObjectMgr->GBKToUtf8("饰品2..."), GOSSIP_SENDER_EQUIPMENT_SHOW, GOSSIP_ACTION_INFO_DEF + BOT_SLOT_TRINKET2);

									  //if (player->IsGameMaster())
									  player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, sObjectMgr->GBKToUtf8("卸下所有装备"), GOSSIP_SENDER_UNEQUIP_ALL, GOSSIP_ACTION_INFO_DEF + 1);

									  player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, sObjectMgr->GBKToUtf8("返回"), 1, GOSSIP_ACTION_INFO_DEF + 1);
	...
	case GOSSIP_SENDER_EQUIPMENT_LIST: //list inventory
	{
										   ...
										   for (uint8 i = 0; i != BOT_INVENTORY_SIZE; ++i)
										   {
											   ...
											   msg << sObjectMgr->GBKToUtf8(" 装备在插槽 ") << uint32(i) << " (" << _getNameForSlot(i + 1) << ')';
											   if (i < BOT_SLOT_RANGED && einfo->ItemEntry[i] == item->GetEntry())
												   msg << sObjectMgr->GBKToUtf8(" |cffe6cc80|h[!标准装备!]|h|r");
	...
	case GOSSIP_SENDER_EQUIPMENT_SHOW: //equips change s2: send list of equippable items
	{
										   ...
										   if (action - GOSSIP_ACTION_INFO_DEF <= BOT_SLOT_RANGED)
											   //if (player->IsGameMaster())
											   player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, sObjectMgr->GBKToUtf8("还是用你的旧装备吧!"), GOSSIP_SENDER_EQUIP_RESET, action);

										   //s2.2.2 add unequip option for non-weapons
										   if (slot >= BOT_SLOT_RANGED && _equips[slot])
											   //if (player->IsGameMaster())
											   player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, sObjectMgr->GBKToUtf8("卸下装备"), GOSSIP_SENDER_UNEQUIP, action);

										   //s2.2.3a: add an empty submenu with info if no items are found
										   if (itemList.empty())
										   {
											   player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, sObjectMgr->GBKToUtf8("啊...我没有装备给你!"), 0, GOSSIP_ACTION_INFO_DEF + 1);
										   }
										   else
										   {
											   uint32 counter = 0;
										   ...
										   player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, sObjectMgr->GBKToUtf8("返回"), GOSSIP_SENDER_EQUIPMENT, GOSSIP_ACTION_INFO_DEF + 2);
	...
	case GOSSIP_SENDER_UNEQUIP: //equips change s3: Unequip DEPRECATED
	{
									if (_unequip(action - (GOSSIP_ACTION_INFO_DEF + 1)))
										BotSay(sObjectMgr->GBKToUtf8("啊..."), player);
									break;
	}
	...
	case GOSSIP_SENDER_EQUIP_AUTOEQUIP:
	{
										  ...
										  if (itemList.empty())
										  {
											  player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, sObjectMgr->GBKToUtf8("啊...我没有什么装备给你!"), 0, GOSSIP_ACTION_INFO_DEF + 1);
										  }
										  else
										  {
											  uint32 counter = 0;
	...
	case GOSSIP_SENDER_ROLES: //ROLES 1: list
	{
								  ...
								  player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, sObjectMgr->GBKToUtf8("返回"), 1, GOSSIP_ACTION_INFO_DEF + role + 1);
	...
	case GOSSIP_SENDER_ABILITIES:
	{
									...
									player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, sObjectMgr->GBKToUtf8("更新"), GOSSIP_SENDER_ABILITIES_USE, GOSSIP_ACTION_INFO_DEF);
									player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, sObjectMgr->GBKToUtf8("返回"), 1, GOSSIP_ACTION_INFO_DEF + 2);
	...
	case GOSSIP_SENDER_HIRE:
	{
							   ...
							   if (!reason)
							   {
								   if (_ownerGuid)
								   {
									   ...
									   if (sObjectMgr->GetPlayerNameByGUID(ObjectGuid(HIGHGUID_PLAYER, _ownerGuid), name))
										   ostr << name;
									   else
	...
	case GOSSIP_SENDER_FORMATION:
	{
									...
									diststr << sObjectMgr->GBKToUtf8("设置距离 (当前: ") << uint32(player->GetBotFollowDist()) << ')';
	...
	case GOSSIP_SENDER_DEBUG:
	{
								...
								ostr << "Bot: " << me->GetName()
									...
									<< "). owner: ";
								if (_ownerGuid && sObjectMgr->GetPlayerNameByGUID(ObjectGuid(HIGHGUID_PLAYER, _ownerGuid), name))
...
char const* bot_minion_ai::_getNameForSlot(uint8 slot) const
{
	...
	case BOT_SLOT_MAINHAND:
		return sObjectMgr->GBKToUtf8("主手武器");
	case BOT_SLOT_OFFHAND:
		return sObjectMgr->GBKToUtf8("副手武器");
	case BOT_SLOT_RANGED:
		return sObjectMgr->GBKToUtf8("远程武器");
	case BOT_SLOT_HEAD:
		return sObjectMgr->GBKToUtf8("头部");
	case BOT_SLOT_SHOULDERS:
		return sObjectMgr->GBKToUtf8("肩部");
	case BOT_SLOT_CHEST:
		return sObjectMgr->GBKToUtf8("胸部");
	case BOT_SLOT_WAIST:
		return sObjectMgr->GBKToUtf8("腰部");
	case BOT_SLOT_LEGS:
		return sObjectMgr->GBKToUtf8("腿部");
	case BOT_SLOT_FEET:
		return sObjectMgr->GBKToUtf8("脚");
	case BOT_SLOT_WRIST:
		return sObjectMgr->GBKToUtf8("手腕");
	case BOT_SLOT_HANDS:
		return sObjectMgr->GBKToUtf8("手");
	case BOT_SLOT_BACK:
		return sObjectMgr->GBKToUtf8("背部");
	case BOT_SLOT_BODY:
		return sObjectMgr->GBKToUtf8("衬衣");
	case BOT_SLOT_FINGER1:
		return sObjectMgr->GBKToUtf8("戒指1");
	case BOT_SLOT_FINGER2:
		return sObjectMgr->GBKToUtf8("戒指2");
	case BOT_SLOT_TRINKET1:
		return sObjectMgr->GBKToUtf8("饰品1");
	case BOT_SLOT_TRINKET2:
		return sObjectMgr->GBKToUtf8("饰品2");
	case BOT_SLOT_NECK:
		return sObjectMgr->GBKToUtf8("颈部");
	default:
		return sObjectMgr->GBKToUtf8("未知");
	}
}
...
char const* bot_ai::GetRoleString(uint8 role) const
{
	...
	case BOT_ROLE_NONE:
		return "???";
	case BOT_ROLE_TANK:
		return sObjectMgr->GBKToUtf8("坦克");
	case BOT_ROLE_DPS:
		return sObjectMgr->GBKToUtf8("输出");
	case BOT_ROLE_HEAL:
		return sObjectMgr->GBKToUtf8("治疗");
		//case BOT_ROLE_MELEE:
		//    return "Melee";
	case BOT_ROLE_RANGED:
		return sObjectMgr->GBKToUtf8("远程");
	default:
	{
			   std::ostringstream str;
			   str << sObjectMgr->GBKToUtf8("角色") << uint32(role);
