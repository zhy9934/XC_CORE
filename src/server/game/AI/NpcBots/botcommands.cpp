...
public:
    ...
    ChatCommand* GetCommands() const
    {
        ...
        {
			{ "faction",            PLAYER_COMMANDS,            false,             &HandleNpcSetFactionCommand, "", NULL },
			{ "owner",              PLAYER_COMMANDS,            false,              &HandleNpcSetOwnerCommand, "", NULL },
            { NULL,         0,                                  false, NULL,                                    "", NULL }
        };

        static ChatCommand npcbotCommandTable[] =
        {
			{ "set", PLAYER_COMMANDS, false, NULL, "", npcbotSetCommandTable },
			{ "add", PLAYER_COMMANDS, false, &HandleNpcBotAddCommand, "", NULL },
			{ "remove", PLAYER_COMMANDS, false, &HandleNpcBotRemoveCommand, "", NULL },
			{ "spawn", PLAYER_COMMANDS, false, &HandleNpcBotSpawnCommand, "", NULL },
			{ "delete", PLAYER_COMMANDS, false, &HandleNpcBotDeleteCommand, "", NULL },
			{ "lookup", PLAYER_COMMANDS, false, &HandleNpcBotLookupCommand, "", NULL },
			{ "revive", PLAYER_COMMANDS, false, &HandleNpcBotReviveCommand, "", NULL },
			{ "cast", PLAYER_COMMANDS, false, &HandleNpcBotCastCustomSpell, "", NULL },
            { NULL,         0,                                  false, NULL,                                    "", NULL }
        };
    ...
    static bool HandleNpcSetOwnerCommand(ChatHandler* handler, const char* args)
    {
        ...
        if (guidlow)
            sObjectMgr->GetPlayerNameByGUID(ObjectGuid(HIGHGUID_PLAYER, guidlow), characterName);
        else
